...
const goSocket = io('http://localhost:3001/go', {
  "transports" : ["websocket"]
});

const phpSocket = io('http://localhost:3001/php', {
  "transports" : ["websocket"]
});
const Game = () => {
  ...
  const generateMoles = (pods) => {
    return pods.filter((pod) => pod !== {}).map((pod) => {
      return {
        name: pod.name,
        ready: pod.running,
        speed: 2,
  ...
  const podNo = useRef(3)
  const [goMoles, setGoMoles] = useState(generateMoles(new Array(0)))
  const [phpMoles, setPhpMoles] = useState(generateMoles(new Array(0)))
  ...
  const [isConnected, setIsConnected] = useState(goSocket.connected)

  const onWhackGo = (points, golden, moleName) => {
    if (!muted) {
      playThud()
    ...
    goSocket.emit("kill_pod", moleName)
    setScore(score + points)
  }

  const onWhackPhp = (points, golden, moleName) => {
    if (!muted) {
      playThud()
      if (golden) playSparkle()
      else {
    ...
    phpSocket.emit("kill_pod", moleName)
    setScore(score + points)
  }
  ...
  const killPod = () => {
    ...
    goSocket.emit("kill_random_pod", "asd")
  }
  ...
  const updateGoMoles = (podList) => {
    setGoMoles(generateMoles(podList))
  }

  const updatePhpMoles = (podList) => {
    setPhpMoles(generateMoles(podList))
  }

  useEffect( () => {
    phpSocket.on("list", (data) => {
      updatePhpMoles(data.podList)
      setTimeout(() => {
        phpSocket.emit("get_pods", "asd")
      }, 500)
    })

    goSocket.on("list", (data) => {
      updateGoMoles(data.podList)
      setTimeout(() => {
        goSocket.emit("get_pods", "asd")
      }, 500)
    })
  ...
  useEffect(() => {
    // connect
    goSocket.on("connect", () => {
      setIsConnected(goSocket.connected)
      goSocket.emit("get_pods", "", (data) => {
        podNo.current = data.total
        updateGoMoles(data.podList)
      })
    })

    phpSocket.on("connect", () => {
      setIsConnected(goSocket.connected)
      phpSocket.emit("get_pods", "", (data) => {
        podNo.current = data.total
        updatePhpMoles(data.podList)
      })
    })

    goSocket.on("connect_error", (reason) => {
      console.log("Error connecting go", reason)
    });

    phpSocket.on("connect_error", (reason) => {
      console.log("Error connecting php", reason)
    });

    goSocket.on("disconnect", (reason) => {
      console.log('Disconnection reason: ' + reason)
      setIsConnected(false)
    })

    phpSocket.on("disconnect", (reason) => {
      console.log('Disconnection reason: ' + reason)
      setIsConnected(false)
    })
    ...
    return () => {
      ...
      phpSocket.emit("bye")
      // connect
      goSocket.off("connect")
      phpSocket.off("connect")
      goSocket.off("pong")
      phpSocket.off("pong")
      goSocket.off("connect_error")
      phpSocket.off("connect_error")
      goSocket.off("disconnect")
      phpSocket.off("disconnect")
      if(goSocket) goSocket.disconnect();
      if(phpSocket) phpSocket.disconnect();
    }
  }, []);
  ...
  return (
    <Fragment>
      ...
      <main className="mole-board" ref={boardRef}>
        <div className="go-moles">
          {goMoles.map(({ speed, delay, points, ready, name }) => (
            <Mole
              key={name}
              name={name}
              onWhack={onWhackGo}
        ...
        <div className="php-moles">
          {phpMoles.map(({ speed, delay, points, ready, name }) => (
            <Mole
              key={name}
              name={name}
              onWhack={onWhackPhp}
